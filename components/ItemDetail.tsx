    ...
    <div className="bg-slate-50 dark:bg-gray-900/50 p-6 border-t border-gray-200 dark:border-gray-700 animate-fadeIn">
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="space-y-6 w-full">
          ...
          {item.type === 'link' && (
              <div className="bg-sky-50 dark:bg-sky-500/10 border border-sky-100 dark:border-sky-900/50 rounded-xl p-6 flex items-center justify-between shadow-sm">
                  <div>
                      <h4 className="text-sky-900 dark:text-sky-300 font-semibold text-lg flex items-center gap-2 mb-1">
                          <Globe size={20} /> Recurso Externo
                      </h4>
                      <p className="text-sky-700/80 dark:text-sky-400/80 text-sm">Este conteúdo está hospedado em um site externo.</p>
                  </div>
                  <a 
                    href={item.url} 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="px-5 py-2.5 bg-sky-600 dark:bg-sky-500 hover:bg-sky-700 dark:hover:bg-sky-600 text-white rounded-lg font-medium flex items-center gap-2 transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                  >
        ...
        <div className="flex flex-col border-t border-gray-300 dark:border-gray-600 pt-8">
          ...
          <div className="space-y-6 mb-8">
            {item.comments.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 border-dashed rounded-xl">
                  <MessageSquare size={40} className="mb-3 text-gray-300 dark:text-gray-600" />
                  <p className="text-gray-500 dark:text-gray-400 font-medium">Ainda não há contribuições.</p>
                  <p className="text-sm text-gray-400 dark:text-gray-500">Seja o primeiro a compartilhar uma solução ou dúvida!</p>
              </div>
            ) : (
                item.comments.map(comment => {
                  if (comment.isDeleted) {
                    if (canDeleteComment(comment)) {
                        return (
                            <div key={comment.id} className="flex gap-4 p-4 bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 rounded-xl opacity-75">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 text-red-800 dark:text-red-400 font-bold text-xs uppercase mb-1">
                                        <AlertCircle size={12} /> Mensagem Removida pela Moderação
                                    </div>
                                    <p className="text-xs text-red-600 dark:text-red-500 italic">Motivo: {comment.deletionReason || "Violação das regras da comunidade."}</p>
                                </div>
                            </div>
                        )
                    } else {
                  ...
                  return (
                  <div key={comment.id} className="flex gap-4 group bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                    <img src={comment.userAvatar} alt={comment.userName} className="w-10 h-10 rounded-full bg-gray-200 object-cover shrink-0 border border-gray-100 dark:border-gray-700" />
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                            <div className="flex items-center gap-2">
                                ...
                                {item.authorId === comment.userId && (
                                    <span className="text-[10px] px-2 py-0.5 bg-indigo-100 dark:bg-gray-700 text-indigo-700 dark:text-indigo-400 rounded-full font-bold uppercase tracking-wider">Autor</span>
                                )}
                            </div>
                            <span className="text-xs text-gray-400 dark:text-gray-500">{new Date(comment.createdAt).toLocaleDateString()} às {new Date(comment.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            {canEditComment(comment) && (
                                <button 
                                    ...
                                    className="p-2 text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-gray-700 rounded-lg transition-colors flex items-center gap-1 text-xs font-medium"
                                    title="Editar"
                                >
                            ...
                            {canDeleteComment(comment) && (
                                <button 
                                    ...
                                    className="p-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-gray-700 rounded-lg transition-colors flex items-center gap-1 text-xs font-medium"
                                    title="Excluir"
                                >
                          ...
                          {comment.screenshotUrl && (
                              <div className="mt-4">
                                  <button 
                                    ...
                                    className="flex items-center gap-3 px-4 py-3 bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 transition-colors group/btn hover:shadow-md w-full sm:w-auto text-left"
                                  >
                                      ...
                                      <div>
                                        <div className="font-semibold dark:text-gray-200">Ver Resultado</div>
                                        <div className="text-xs text-gray-500 dark:text-gray-400">Clique para ampliar o screenshot</div>
                                      </div>
          ...
          {currentUser ? (
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
                <div className="p-4 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700 flex items-center gap-2">
                    ...
                    <h5 className="font-semibold text-gray-700 dark:text-gray-300 text-sm">Adicionar Contribuição</h5>
                </div>
                ...
                {newCommentScreenshot && (
                    <div className="mb-4 px-1">
                        <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2">Imagem Anexada:</p>
                        <div className="relative inline-block group">
                            <img src={newCommentScreenshot} alt="Preview" className="h-24 w-auto rounded-lg border border-gray-200 dark:border-gray-600 shadow-sm" />
                            <button 
                                type="button"
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
                    <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
                        <button 
                            ...
                            className="px-3 py-1.5 text-gray-600 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-gray-700 bg-white dark:bg-gray-900/50 border border-gray-200 dark:border-gray-600 hover:border-indigo-200 rounded-md transition-colors flex items-center gap-2 text-xs font-medium shadow-sm"
                    ...
                    <button 
                        ...
                        className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 disabled:bg-gray-300 dark:disabled:bg-gray-600 disabled:text-gray-500 dark:disabled:text-gray-400 text-white text-sm font-medium rounded-lg transition-all transform active:scale-95 shadow-md"
                    >
                    ...
                    <div className="mt-3 animate-fadeIn">
                        <div className="flex items-center gap-2 bg-slate-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-1 focus-within:border-indigo-400 focus-within:ring-1 focus-within:ring-indigo-400 transition-all">
                            ...
                            <input 
                                ...
                                className="w-full py-2 bg-transparent border-none outline-none text-xs text-gray-600 dark:text-gray-300 placeholder-gray-400 dark:placeholder-gray-500"
                            />
                        </div>
                    </div>
                )}
            ...
            <div className="text-center p-8 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
              <p className="text-gray-500 dark:text-gray-300 font-medium mb-2">Faça login para participar da discussão.</p>
              <p className="text-sm text-gray-400 dark:text-gray-500">Você precisa de uma conta aprovada para postar.</p>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4 animate-fadeIn">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
                ...
                <form onSubmit={handleSaveEdit} className="flex-1 flex flex-col p-6 overflow-y-auto custom-scrollbar">
                    ...
                    {/* Edit Media Preview */}
                    <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Imagem Anexada</label>
                        {editCommentScreenshot ? (
                            <div className="relative inline-block group">
                                <img src={editCommentScreenshot} alt="Preview" className="h-32 w-auto rounded-lg border border-gray-200 dark:border-gray-600 shadow-sm" />
                                <button 
                                    type="button"
                            ...
                            <div className="p-4 border-2 border-dashed border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700/50 text-center">
                                <p className="text-xs text-gray-400 dark:text-gray-500">Nenhuma imagem anexada</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                        <button 
                            ...
                            className="px-3 py-1.5 text-gray-600 dark:text-gray-300 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-amber-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-md transition-colors flex items-center gap-2 text-xs font-medium"
                        >
                            <Code size={14} /> Inserir Código
                        </button>
                    ...
                    {showEditUrlInput && (
                        <div className="mb-4 animate-fadeIn">
                            <div className="flex items-center gap-2 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-1 focus-within:border-amber-400 focus-within:ring-1 focus-within:ring-amber-400">
                                <LinkIcon size={14} className="text-gray-400 dark:text-gray-500" />
                                <input 
                                    type="url"
                                    ...
                                    className="w-full py-2 bg-transparent border-none outline-none text-xs text-gray-600 dark:text-gray-300"
                                />
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-gray-700 mt-auto">
                        ...
                        <button 
                            ...
                            className="px-6 py-2 bg-amber-600 hover:bg-amber-700 disabled:bg-gray-300 dark:disabled:bg-gray-600 text-white font-medium rounded-lg shadow-md transition-all flex items-center gap-2 text-sm"
                        >
