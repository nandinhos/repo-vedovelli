...
export default function App() {
  const [usersDb, setUsersDb] = useState<User[]>(INITIAL_USERS_DB);
  const [currentUser, setCurrentUser] = useState<User | null>(null);
  const [viewingUser, setViewingUser] = useState<User | null>(null); // For viewing other profiles
  
  const [items, setItems] = useState<RepositoryItem[]>(INITIAL_ITEMS);
  ...
  const handleSubmitItem = (e: React.FormEvent) => {
    ...
    const baseItem = {
      ...
      authorName: editingItem ? editingItem.authorName : currentUser.name,
  ...
  const handleAddComment = (itemId: string, text: string, screenshot?: string) => {
    if (!currentUser) return;
    const newComment = {
  ...
  const handleEditComment = (itemId: string, commentId: string, text: string, screenshot?: string) => {
      setItems(prev => prev.map(item => {
          if (item.id === itemId) {
              return {
                  ...item,
  ...
  const confirmAdminDeletion = () => {
      ...
      setDeleteReasonModal({ isOpen: false, itemId: '', commentId: '', reason: '' });
  };
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex justify-end gap-3">
              ...
              <button 
                ...
                className={`px-6 py-2.5 text-white font-medium rounded-lg shadow-lg transition-all transform active:scale-95 ${
                    uploadType === 'file' ? 'bg-emerald-600 hover:bg-emerald-700' :
